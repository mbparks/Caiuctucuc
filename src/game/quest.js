...
export const EVIDENCE = {
  ...
  calm_bootprints: 1, razored_ledger: 1, gentleman_letter: 1, he_measures: 1
};
...
export function threadDone(state) {
  ...
  if (has('razored_ledger') && has('plat_mismatch')) return 'plat';
...
export const BEATS = [
  ...
  {
    id: 'gossipturn',
    when: s => !s.flags.gossipTurn && s.clues.filter(c => ['singing_confession','calm_bootprints','razored_ledger','plat_mismatch','gentleman_letter'].includes(c)).length >= 2,
    apply: s => ({
  ...
  {
    id: 'ambush',
    when: s => s.flags.ganttProvoked && !s.flags.ambushDone,
    apply: s => {
      const storied = s.difficulty?.combat === 'storied';
      const health = storied ? s.player.health : Math.max(1, s.player.health - 3);
      return {
        state: { ...s, player: { ...s.player, health },
                 flags: { ...s.flags, ambushDone: true, survivedAmbush: true, beallCertain: true } },
        toast: 'The Narrows overlook, dusk. A surveyor\u2019s chain across the path and a reasonable man behind it doing an unreasonable thing. You survive it, barely, and Beall stops needing convincing: nobody ambushes a stranger over mathematics.'
      };
    }
  },
  {
    id: 'elderswaver',
    when: s => s.flags.act2Complete && !s.flags.verdict && !s.flags.eldersWaver && (s.flags.sleepCount || 0) > (s.flags.act2SleepMark ?? 1e9) + 3,
    apply: s => ({
      state: { ...s, flags: { ...s.flags, eldersWaver: true } },
      toast: 'Word from the courthouse: Gantt has been closeted with Cresap and Kent for hours, and came out looking like a man who offered something. If the case is going to court, it should go soon.'
    })
  },
  {
    id: 'act3',
    when: s => Boolean(s.flags.verdict) && !s.flags.act3Complete,
    apply: s => ({
      state: { ...s, flags: { ...s.flags, act3Complete: true, act3SleepMark: s.flags.sleepCount || 0 } },
      toast: s.flags.verdict === 'guilty'
        ? 'ACT III CLOSES: Prosper Gantt hangs on the strength of what you gathered. The town breathes out. The digging, you notice, has not stopped.'
        : s.flags.verdict === 'deal'
          ? 'ACT III CLOSES: the case is buried with civic efficiency. Gantt re-surveys the line, the quarry stays legal, the wagons keep rolling, and everyone agrees not to remember. A peace with poison in it.'
          : 'ACT III CLOSES: acquitted. Gantt shakes hands on the courthouse steps and holds yours one beat too long. Gather more, and lay it again.'
    })
  },
  {
    id: 'nanmissing',
    when: s => s.flags.act3Complete && !s.flags.nanMissing && (s.flags.sleepCount || 0) > (s.flags.act3SleepMark ?? 1e9),
    apply: s => ({
      state: addClue({ ...s, flags: { ...s.flags, nanMissing: true } }, 'nan_missing'),
      toast: 'The mill wheel has stopped. Livestock in the night, and then Nan Trent, gone from a locked room. Every dog, cat, and mule in the county is looking at the mountain, and Robey Marsh, stone sober, has nailed a horseshoe over the cave mouth on the quarry road: the way in, marked.'
    })
  },
  {
    id: 'act4close',
    when: s => Boolean(s.flags.ending) && !s.flags.act4Complete,
    apply: s => ({
      state: { ...s, flags: { ...s.flags, act4Complete: true } },
      toast: s.flags.ending === 'buried_truth'
        ? 'THE BURIED TRUTH. What sleeps under Wills Mountain is put down properly, with rites older than the county, by someone who paid to learn how to listen. Nan walks out holding your hand. The town is told a cave collapsed. The epilogue is yours to walk.'
        : 'THE LEDGER CLOSED. The cave is sealed with powder and civic silence, Nan carried out asleep on your shoulder, and the town calls it settled. Next spring the quarry reopens, one cut to the north. The epilogue is yours to walk.'
    })
  },
  {
    id: 'act1',
    when: s => s.flags.coombsDead && !s.flags.act1Complete,
    apply: s => ({
      state: { ...s, flags: { ...s.flags, act1Complete: true, act1SleepMark: s.flags.sleepCount || 0 } },
      toast: 'ACT I CLOSES: The Drowned Man. Two bodies, one thread in your hand, and a town beginning to choose fear. Act II waits past the end of this build.'
    })
  }
];

// The chamber: two endings from one branch point. Design doc Section 18.
export const ENDINGS = {
  ledger_closed: { name: 'The Ledger Closed', gate: () => true },
  buried_truth: { name: 'The Buried Truth', gate: s => s.player.sight >= 2 }
};

export function chooseEnding(state, which) {
  const e = ENDINGS[which];
  if (!e) throw new Error('no such ending: ' + which);
  if (!state.flags.inChamber) return { ok: false, reason: 'the chamber is a long way down' };
  if (state.flags.ending) return { ok: false, reason: 'the choice is made' };
  if (!e.gate(state)) return { ok: false, reason: 'that ending asks for the Listening, and you never opened the second door' };
  return { ok: true, state: { ...state, flags: { ...state.flags, ending: which, nanSaved: true } } };
}

// Companions: one at a time; the others remember.
export function adoptPet(state, pet) {
  if (state.pet === pet) return { ok: false, reason: 'already at your heel' };
  return { ok: true, state: { ...state, pet } };
}

export function advanceCase(state) {
  const toasts = [];
  let s = state, fired = true;
  while (fired) {
    fired = false;
...
// Dialog confessions become clues.
export const FLAG_CLUES = { mcteagueConfessed: 'singing_confession', roodShared: 'razored_ledger', bootsMatched: 'boots_matched', hasRubbings: 'rubbings_matched', benchmarksAll: 'true_line' };
export function cluesFromFlags(state) {
  let s = state;
  for (const [flag, clue] of Object.entries(FLAG_CLUES))
    if (s.flags[flag]) s = addClue(s, clue);
  return s;
}
