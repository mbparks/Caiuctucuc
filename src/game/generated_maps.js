...
function canal() {
  return makeMap(42, 26, 9, [[0, 15, 42, 5, 2], [4, 5, 34, 2, 2]], [[5, 4, 32], [20, 4, 23], [30, 9, 106], [12, 10, 107]],
    ...
    [door(200, 'road back to town', 32, 96, 'town', 'from_canal'), door(201, 'B&O rail yard', 608, 96, 'rail_yard', 'from_canal'), door(202, 'Cumberland Quarry road', 336, 32, 'quarry', 'from_canal')],
    [
...
function railYard() {
  return makeMap(44, 24, 11, [], [[8, 8, 98], [9, 8, 99], [20, 8, 98], [21, 8, 99], [32, 8, 98], [33, 8, 99]],
    ...
    [door(200, 'canal basin', 32, 96, 'canal', 'from_rail_yard'), door(201, 'Cumberland Quarry spur', 640, 96, 'quarry', 'from_rail_yard')],
    [
...
function quarry() {
  return makeMap(46, 28, 12, [[6, 20, 16, 2, 2]], [[12, 7, 23], [15, 7, 23], [18, 7, 23], [22, 8, 32], [30, 14, 106], [31, 14, 107]],
    ...
    [door(200, 'canal road', 32, 96, 'canal', 'from_quarry'), door(201, 'rail spur', 80, 80, 'rail_yard', 'from_quarry'), door(202, 'Wills Mountain path', 688, 160, 'wills_mountain', 'from_quarry')],
    [
      obj(300, 'Wills Creek Formation face', 'clue', 240, 112, 16, 16, [prop('clue', 'singing_confession'), prop('teaches', 'QUARRY,SINGING'), prop('text', 'Your palm on the Wills Creek Formation cut finds a vibration too even for wind. Men heard it singing before Tam went missing.')]),
      ...
      obj(304, 'benchmark in Cumberland Quarry', 'benchmark', 608, 176),
      obj(305, 'powder shed', 'oddity', 448, 176, 32, 16, [prop('text', 'Powder casks, survey stakes, and a new lock. Cumberland Quarry is a business pretending not to be a crime scene.')])
    ]);
}
...
function willsMountain() {
  return makeMap(44, 30, 13, [[0, 24, 44, 2, 2]], [[10, 10, 32], [18, 12, 32], [24, 7, 32], [28, 16, 23], [34, 20, 23]],
    ...
    [door(200, 'Cumberland Quarry road', 32, 96, 'quarry', 'from_wills_mountain'), door(201, 'Cumberland Bone Cave approach', 640, 128, 'cave_chain', 'from_wills_mountain')],
    [
      obj(300, 'animal track convergence', 'clue', 208, 128, 16, 16, [prop('requires', 'act3Complete'), prop('clue', 'nan_missing'), prop('sets', 'nanMissing'), prop('teaches', 'NAN,MOUNTAIN'), prop('text', 'Every hoofprint and pawmark points north. The animals knew before the town did: Nan Trent has gone where Wills Mountain listens.')]),
...
function caveChain() {
  return makeMap(36, 24, 14, [[10, 10, 16, 2, 2]], [[12, 8, 23], [18, 8, 23], [24, 8, 23], [20, 16, 32]],
    ...
    [door(200, 'Wills Mountain mouth', 32, 80, 'wills_mountain', 'from_cave_chain'), door(201, 'Cumberland Bone Cave', 512, 240, 'cathedral', 'from_cave_chain')],
    [
      ...
      obj(301, 'Keyser limestone seam', 'murmur', 256, 128, 16, 16, [prop('rank', 2), prop('teaches', 'SEAL,WINONA'), prop('text', 'A sealed seam under Cumberland. Names older than deeds press their hands against the Keyser limestone dark.')]),
...
function cathedral() {
  return makeMap(34, 22, 15, [[8, 16, 18, 2, 2]], [[12, 6, 32], [18, 6, 32], [22, 6, 32], [17, 10, 23]],
    ...
    [
      obj(300, 'Nan at the Bone Cave center', 'chamber', 272, 144),
      obj(301, 'Cumberland Bone Cave dark', 'clue', 240, 112, 16, 16, [prop('requires', 'nanMissing'), prop('clue', 'the_chamber'), prop('teaches', 'NAN,SEAL'), prop('text', 'Nan sits unharmed in a dark too large for Wills Mountain. She is listening, and what answers her is older than the county.')]),
